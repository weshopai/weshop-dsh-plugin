...
`;
		...
		*/
		...
		function WeshopOpenAction({ onOpen, sessions }) {
			const state = (0, react.useSyncExternalStore)((listener) => sessions.list.subscribe(listener), () => sessions.list.getSnapshot(), () => sessions.list.getSnapshot());
			if ((state.current === void 0 ? void 0 : state.byId[state.current])?.agentPreset !== WESHOP_PRESET) return null;
		...
		function apply(ctx) {
			...
			const openPanel = (initialActionCursor = Date.now()) => {
				...
				const current = sessionId === void 0 ? void 0 : state.byId[sessionId];
				const binding = sessionId === void 0 ? void 0 : ctx.sessions.binding(sessionId);
				if (sessionId === void 0 || binding === void 0 || current?.agentPreset !== WESHOP_PRESET) return;
