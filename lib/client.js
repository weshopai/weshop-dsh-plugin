		...
		*/
		...
		function ModeHint({ enabled }) {
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("strong", { children: enabled ? zh ? "WeShop 画布已启用" : "WeShop canvas enabled" : zh ? "已切回标准模式" : "Standard mode restored" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("small", { children: enabled ? zh ? "点击左下角画布按钮即可随时打开" : "Use the canvas button at the lower left to open it anytime" : zh ? "画布入口已收起" : "The canvas shortcut is now hidden" })] })]
			});
		}
		...
		function apply(ctx) {
			...
			const showModeHint = (enabled) => {
				...
				disposeModeHint = ctx.slots.register({
					...
					id: "weshop-mode-hint",
					order: 90
				}, () => /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ModeHint, { enabled }));
			...
			const presetSelected = ctx.remote.$on("agent-preset/selected", (sessionId, agentPreset) => {
				...
				sync();
				if (ctx.sessions.list.getSnapshot().current === sessionId && previous !== agentPreset) showModeHint(agentPreset === WESHOP_PRESET);
			});
			sync();
